 ...
 */
public class JedisPoolUtil implements RedisPool {
    private final static String PROPERTIES_PATH = "redis.properties";
    ...
    public JedisPool getRedisPool() {
        return Instance.redisPool.getRedisPool();
    }
    ...
    public void init() throws Exception {
        ...
        int port = StringUtils.isEmpty(p.getProperty("redis.port"))? 6379 : Integer.parseInt(p.getProperty("redis.port"));
        String auth = p.getProperty("redis.auth");
        int poolTimeOut = StringUtils.isEmpty(p.getProperty("connectionTimeOut")) ? 2000
    ...
    static class Instance {
        static JedisPoolUtil redisPool = new JedisPoolUtil();
    }
}
